...
class CustomEnv(gym.Env):
    ...
    def step(self, action):
        ...
        def ode(t, s):
            x, y = s
            Vx = self.A * (- np.sin(2 * np.pi * x + self.B * np.sin(2 * np.pi * self.omega * t)) * np.cos(2 * np.pi * y))
            Vy = self.A * ( np.cos(2 * np.pi * x + self.B * np.sin(2 * np.pi * self.omega * t)) * np.sin(2 * np.pi * y))
