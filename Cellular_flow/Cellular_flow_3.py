...
class TD3Agent:
    ...
    def select_action(self, state, add_noise=True):
        ...
        if add_noise:
            ...
            action = action + noise
        norm = np.linalg.norm(action) + 1e-8
        action = action / norm
        return action
...
class CustomEnv(gym.Env):
    ...
    def reset(self):
        ...
        Vx = -16 * np.sin(2 * np.pi * self.x) * np.cos(2 * np.pi * self.y)
        Vy = 16 * np.cos(2 * np.pi * self.x) * np.sin(2 * np.pi * self.y)
        
        return np.array([self.x, self.y, Vx, Vy], dtype=np.float32)
    ...
    def step(self, action):
        ...
        return np.array([self.x, self.y, Vx, Vy], dtype=np.float32), reward, done, {}
        ...
        for x in xs:
            for y in ys:
                ...
                agent.replay_buffer.add(
                    np.array([x, y, Vx, Vy], dtype=np.float32), 
                    action, 
                    reward, 
                    next_state, 
                    done
